...
export default function DeleteComment({ isOpen, onOpenChange, comment }) {
  ...
  function delComment(data) {
    setLoading(true);
    axios
      .delete(`https://linked-posts.routemisr.com/comments/${comment._id}`, {
        headers: {
          token: userToken,
        },
      })
        ...
        <ModalContent>
          {(onClose) => (
            <form onSubmit={handleSubmit(delComment)}>
              ...
              <ModalBody>
                <p className="mb-3">
                  Are your sure you want to delete comment?{" "}
                </p>
              </ModalBody>
              <ModalFooter className="border-t-1">
                ...
                >
                  {Loading ? (
                    ...
                    <RiDeleteBin6Line className="size-5 text-zinc-900 cursor-pointer" />
                  )}
                </Button>
              </ModalFooter>
            </form>
